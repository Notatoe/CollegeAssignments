...
async function setupScene() {
  ...
  let vertSource3 = await loadNetworkResourceAsText('resources/shaders/verts/midterm.vert');
  let fragSource3 = await loadNetworkResourceAsText('resources/shaders/frags/midterm.frag');
  ...
  let objData5 = await loadNetworkResourceAsText('resources/models/textureSphere.obj');
  let vertSource5 = await loadNetworkResourceAsText('resources/shaders/verts/enviromentMap.vert');
  let fragSource5 = await loadNetworkResourceAsText('resources/shaders/frags/enviromentMap.frag');
  ...
  Objects.push(initializeMyBumpmap(vertSource3, fragSource3, objData3));
  Objects.push(initializeMyCubemap(vertSource4, fragSource4, objData4));
  Objects.push(initializeMyCubemap(vertSource5, fragSource5, objData5));
}
...
function drawScene(deltaTime) {
  ...
  { 
    ...
    if(i == 1)
    {
      objectWorldPos = [0.0, -1.5, 0.0];
    }
    if(i == 2)
    {
      objectWorldPos = [0.0, -2.99, 0.0];
    }
    if(i == 4)
    {
      objectWorldPos = [0.0, 2, 0.0];
    }
    ...
    if(cameraPos[2] >= 10.0)
    {
      var fogAmount = cameraPos[2] - 10.0;
      fog = .1 * fogAmount;
    }
    else if(cameraPos[2] <= -10.0)
    {
      var fogAmount = (cameraPos[2] * -1) - 10.0;
      fog = .1 * fogAmount;
    }
    else
    {
